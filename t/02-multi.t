...
@res = qx{./mort perl -e 'fork&&exit 0;exit 1'};
is scalar @res, 2, "got two results";
is $?>>8, 0, "success";
is grep(/exit 0$/, @res), 1;
is grep(/exit 1$/, @res), 1;

@res = qx{./mort perl -e 'fork&&exit 1;exit 2'};
is scalar @res, 2, "got two results";
is $?>>8, 1, "one";
is grep(/exit 1$/, @res), 1;
is grep(/exit 2$/, @res), 1;

@res = qx{./mort perl -e 'fork&&exit 0;for my \$x (1,2,3) {fork||exit\$x}'};
...
@res = qx{./mort perl -e 'fork&&exit 0;my \$k = fork; if(\$k){kill TERM=>\$k} else{sleep 10}'};
